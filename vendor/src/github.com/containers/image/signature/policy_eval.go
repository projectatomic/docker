...
type PolicyRequirement interface {
	...
	isImageAuthenticated(image types.UnparsedImage) (bool, error)
...
// nor does not necessarily mean that the image is trusted to affect the local system (e.g.
// to run as root, or to affect system or cluster configuration.)
func (pc *PolicyContext) IsImageAuthenticated(image types.UnparsedImage) (res bool, finalErr error) {
	if err := pc.changeState(pcReady, pcInUse); err != nil {
		return false, err
	}
	defer func() {
	...
	for reqNumber, req := range reqs {
		// FIXME: supply state
		authenticated, err := req.isImageAuthenticated(image)
		if authenticated {
			res = true
