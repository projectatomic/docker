...
func (pr *prInsecureAcceptAnything) isImageAuthenticated(image types.UnparsedImage) (bool, error) {
	// prInsecureAcceptAnything semantics: Every image is allowed to run,
	// but this does not consider the signature as verified.
	return false, nil
}
...
func (pr *prReject) isImageAuthenticated(image types.UnparsedImage) (bool, error) {
	return false, PolicyRequirementError(fmt.Sprintf("Authenticating any image %s is rejected by policy.", transports.ImageName(image.Reference())))
}
