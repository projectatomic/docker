	...
	for _, tc := range []struct {
		...
		},
		{
			...
			metadata: []metadata.V2Metadata{
				...
				{Digest: digest.Digest("2"), SourceRepository: "user/bar"},
				{Digest: digest.Digest("3"), SourceRepository: "docker.io/library/app"},
				{Digest: digest.Digest("4"), SourceRepository: "docker.io/user/bar"},
			},
			candidates: []metadata.V2Metadata{
				{Digest: digest.Digest("4"), SourceRepository: "docker.io/user/bar"},
				{Digest: digest.Digest("2"), SourceRepository: "user/bar"},
		...
		ref, err := reference.ParseNamed(tc.targetRepo)
		if err != nil {
			t.Fatalf("[%s] failed to parse reference name: %v", tc.name, err)
		}
		repoInfo, err := registry.ParseRepositoryInfo(ref)
		if err != nil {
			t.Fatal("[%s] failed to parse reference: %v", tc.name, err)
		}
...
func TestLayerAlreadyExists(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			remoteErrors:       map[digest.Digest]error{digest.Digest("apple"): distribution.ErrUnsupported},
			expectedError:      distribution.ErrUnsupported,
		...
		},
		{
			...
			maxExistenceChecks: 3,
			remoteErrors:       map[digest.Digest]error{"orange": distribution.ErrUnsupported},
...
func (m *mockRepo) Name() string {
	m.t.Fatalf("Name() not implemented")
	return ""
}
